# -*- coding: utf-8 -*-
"""
Created on Tue Apr 14 20:45:36 2020

@author: SMadhavan
"""

#import progressbar as prg
import excel_interface as ei
import plotter_caseB as pl

  
def mindex_grouped_df(df, index_by):
    df.set_index(index_by, inplace = True)
    dfg = df.groupby(level = [0,1])
    return dfg
    

if __name__=="__main__":
    # =========================================================================
    # INPUTS - Please read the comments noted next to inputs.
    # =========================================================================
    # Note: Do not remove the 'r' in front of excel filepath. It indicates to 
    # the program that the filepath is in raw-string format. Just copy paste 
    # the filepath and place it within " ".
    excel_filepath = r""
    sheet_name = 'Collector Forces X' # Name of the excel tab
    start_row_no = 60   # This is the row number in excel (incl. header row)
    end_row_no = 283    # Row number of the last data point
    start_col_no = "A"  # Column label of first column of data
    end_col_no = "G"    # Column label of last column of data
    fig_folder = 'Collector Diagrams X' # This folder has to exist with this 
                                        # name, where your script lives
    
    index_by = ['Grid', 'Level']    # Caution! This is used to group output
                                    # Modify only if you know what you're doing
    # =========================================================================
    # =========================================================================
    
    data_range = ei.get_excel_range(start_row_no, end_row_no, start_col_no, end_col_no)
    df = ei.get_df_from_input(excel_filepath, sheet_name, data_range)
    dfg = mindex_grouped_df(df, index_by)
    #bar = prg.ProgressBar(max_value=len(dfg))
    progress_counter = 0
    for name, group in dfg:
        gridline, story = name
        pl.plot_collector_diagram(gridline, story, group, 
                                  fig_folderpath = fig_folder, 
                                  save_fig = True, close_fig = True)
        progress_counter +=1
        #bar.update(progress_counter)
    
    print("Success!")
